...
select a.nome, a.data_nascimento
from alunos a
where year (data_nascimento) >= '2000-01-01';
...
-- opção 1
select max(n.nota) as soma_notas
from notas n
inner join disciplinas d on d.id_disciplina = n.id_disciplina 
...
select d.nome as disciplinas, count(m.id_aluno)
from disciplinas d 
...
-- exercico 21
select  max(nota) as maior_notas from notas;

-- exercico 22
select  min(nota) as menor_notas from notas;

-- exercico 23
select  avg(nota) as media from notas;

-- exercico 24
select d.id_disciplina, d.nome as disciplina, avg(n.nota) as media
from disciplinas d 
